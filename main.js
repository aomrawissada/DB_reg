...
////////////////////////////TEACHER FUNCTION////////////////////////////////////////
	...
	function f09(){ //ผลการเรียนรายบุคคล
	    clearelement();
		document.getElementById("caption").innerHTML = "ผลการเรียนรายบุคคล";
	...
	function f09_1(){ //ผลการเรียนรายบุคคล
		var check=0;
		fetch("http://localhost:3000/getAllstudent")
			...
			.then(function (myJson) {
				console.log(myJson);
				for(i=0;i<myJson.length;i++){
					if(myJson[i].SID==document.getElementById("studentID").value) {
						if(myJson[i].AdvisorTID==loginID) {
							check=1;
							document.getElementById(c0[1]).innerHTML = myJson[i].SID+" "+myJson[i].Name+ " "+myJson[i].Surname;
							f09_2(myJson[i].SID);
							break;
						}
						else {
							check=1;
							alert("You can only view grade of student whom you are his/her adviser"); 
							break;
						}
					}
				}
	...
	function f09_2(studentID){ //ผลการศึกษา
		fetch("http://localhost:3000/getquerygrade/sid="+studentID)
			...
			.then(function (myJson) {
				count = 2;
				for(i=0;i<myJson.length;i++){
					f09_3(studentID,myJson[i].CID);
				}
		});
	}
	function f09_3(studentID,courseID){
		fetch("http://localhost:3000/getquerycourse/cid="+courseID)
		.then(function (response) {
			console.log("return2")
			return response.json();
		...
		.then(function (myJson) {
			for(i=0;i<myJson.length;i++){
				count = (count+1)
				document.getElementById(c0[count]).innerHTML = courseID+" "+myJson[i].CName;
				document.getElementById(c0[count]).onclick = f09_4(studentID,courseID,c0[count]);
			}
		})					
	}
	function f09_4(studentID,courseID,elementID){
		fetch("http://localhost:3000/getquerygrade/cid="+courseID)
		.then(function (response) {
			console.log("return2")
			return response.json();
	...
	function f11_2(studentID, courseID, semes, newgrade){ //ผลการเรียนรายบุคคล
		var check2 = 0;
		fetch("http://localhost:3000/getquerygrade/cid="+courseID)
		.then(function (response) {
			console.log("return2")
